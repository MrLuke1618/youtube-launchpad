...
export const ThumbnailTester: React.FC<ThumbnailTesterProps> = ({ initialData, onManageApiKey }) => {
    ...
    const handleGenerateImage = async (setter: 'A' | 'B', isRetry: boolean = false) => {
        ...
        try {
            ...
            setError(e.message || `An AI error occurred while generating the image for Combination ${setter}. Please try again.`);
        } finally {
            setIsGenerating(false);
        }
    };

    const clearImage = (setter: 'A' | 'B') => {
        if (setter === 'A') {
            setImageA(null);
            setPreviewA(null);
            setPromptA(null);
        ...
        return (
            <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50 p-4" onClick={onClose}>
                <div className="bg-dark-card rounded-lg border border-dark-border shadow-xl w-full max-w-4xl text-white transform transition-all flex flex-col max-h-[90vh]" onClick={e => e.stopPropagation()}>
                    ...
                    {data.prompt && (
                        <div className="p-4 border-y border-dark-border bg-dark-bg text-xs font-mono text-dark-text-secondary relative">
                           <p><strong className="font-sans font-semibold text-dark-text">AI Prompt used:</strong> {data.prompt}</p>
                           <button onClick={handleLocalCopy} className="absolute top-3 right-3 p-1 bg-dark-card rounded-md text-dark-text-secondary hover:text-white transition-colors">
                                {localCopied ? <CheckCircleIcon className="w-5 h-5 text-green-400"/> : <ClipboardIcon className="w-5 h-5"/>}
                           </button>
                        </div>
                    )}
    ...
    return (
        <div className="space-y-8 animate-fade-in">
                        ...
                        return (
                             <div key={setter} className="space-y-3">
                                ...
                                >
                                    {isGeneratingImage ? (
                                        ...
                                        <>
                                            ...
                                            <div className="absolute top-2 right-2 flex flex-col gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                                {prompt && (
                                                    <button onClick={() => handleGenerateImage(setter, true)} title="Retry AI generation" className="p-2 bg-black/50 rounded-full text-white hover:bg-brand-purple"><SparklesIcon className="w-5 h-5"/></button>
                                                )}
                                                <button onClick={() => clearImage(setter)} title="Clear image" className="p-2 bg-black/50 rounded-full text-white hover:bg-red-500"><TrashIcon className="w-5 h-5"/></button>
                                            </div>
                                        </>
                                    ) : (
                                        <div className="text-center text-dark-text-secondary p-4 space-y-3">
                                            <button onClick={() => handleGenerateImage(setter, false)} className="flex items-center gap-2 mx-auto text-sm font-semibold hover:text-brand-purple-light">
                                                <SparklesIcon className="w-5 h-5" /> Generate a Thumbnail
                                            </button>
                                    ...
                                    <input type="file" ref={fileInputRef} onChange={(e) => handleImageChange(e, setter)} className="hidden" accept="image/png, image/jpeg, image/webp" />
                                </div>
                                {prompt && (
                                    <div className="relative bg-dark-bg p-2 rounded-md border border-dark-border text-xs text-dark-text-secondary font-mono">
                                        <p><span className="font-sans font-bold text-dark-text">AI Prompt:</span> {prompt}</p>
                                        <button onClick={() => handleCopyPrompt(prompt, setter)} title="Copy prompt" className="absolute top-1.5 right-1.5 p-1 bg-dark-card rounded-md text-dark-text-secondary hover:text-white transition-colors">
                                            {copiedPrompt === setter ? <CheckCircleIcon className="w-5 h-5 text-green-400"/> : <ClipboardIcon className="w-5 h-5"/>}
                                        </button>
                                    </div>
                                )}
                            </div>
            ...
            <div className="flex flex-col items-center gap-4">
                ...
                >
                    <BeakerIcon className="h-5 w-5"/> {isLoading ? 'Analyzing...' : 'Analyze & Predict Winner'}
                </button>
            ...
            {analysis && (
                 <div ref={reportContentRef} className="space-y-6">
                    <div className="text-center bg-dark-card p-6 rounded-lg border border-dark-border">
                        <h2 className="text-sm font-semibold uppercase text-dark-text-secondary">Predicted Winner</h2>
                        <p className={`text-4xl font-bold mt-1 ${analysis.predictedWinner === 'Tie' ? 'text-white' : 'text-brand-purple-light'}`}>
                            {analysis.predictedWinner === 'Tie' ? "It's a Tie!" : `Combination ${analysis.predictedWinner}`}
                        </p>
                        <div className="relative max-w-2xl mx-auto mt-2">
                             <textarea
                                value={analysis.winnerReasoning}
                                onChange={(e) => handleAnalysisChange('winnerReasoning', e.target.value)}
                                className="w-full text-sm text-dark-text-secondary bg-transparent focus:outline-none focus:bg-dark-bg p-1 rounded-md text-center pr-8"
                                rows={2}
                             />
                            <button onClick={() => handleSuggestAnalysis('winnerReasoning', 'Winner Reasoning', analysis.winnerReasoning, `Based on a comparison of two thumbnails/titles for a video on ${topic}.`)} disabled={loadingSuggestions['winnerReasoning']} title="Suggest reasoning" className="absolute top-1 right-2 text-dark-text-secondary hover:text-brand-purple-light disabled:opacity-50"><SparklesIcon className="w-5 h-5"/></button>
                        </div>
                    </div>

                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <AnalysisCard title="Combination A Analysis" analysis={analysis.analysisA} isWinner={analysis.predictedWinner === 'A'} cardKey="analysisA" />
                        <AnalysisCard title="Combination B Analysis" analysis={analysis.analysisB} isWinner={analysis.predictedWinner === 'B'} cardKey="analysisB" />
                    </div>
                    
                    <div className="bg-dark-card p-6 rounded-lg border border-dark-border">
                         <div className="flex justify-between items-center mb-2">
                            <h3 className="text-lg font-bold text-brand-purple-light">The Power of a High CTR</h3>
                             <button onClick={() => handleSuggestAnalysis('ctrContext', 'CTR Context', analysis.ctrContext, `Explain why a high CTR is important for YouTube channel growth.`)} disabled={loadingSuggestions['ctrContext']} title="Suggest context" className="p-1 text-dark-text-secondary hover:text-brand-purple-light disabled:opacity-50"><SparklesIcon className="w-5 h-5"/></button>
                        </div>
                         <textarea
                            value={analysis.ctrContext}
                            onChange={(e) => handleAnalysisChange('ctrContext', e.target.value)}
